...
func (p *Plugin) InitWeb() {
	...
	// either server the group page or the index page, kinda convoluted but eh
...
func HandleGetIndex(w http.ResponseWriter, r *http.Request) (tmpl web.TemplateData, err error) {
	...
	ungroupedCommands, err := models.RoleCommandsG(qm.Where("guild_id = ?", g.ID), qm.Where("role_group_id = null")).All()
	if err != nil {
		return tmpl, err
	}
